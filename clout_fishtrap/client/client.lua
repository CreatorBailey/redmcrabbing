...
Citizen.CreateThread(function()
    local mashTime = 65  * 1800
    --exports['progressBars']:startUI(mashTime, "Waiting For Trap To Shut . . .")
    ...
    while true do
        ...
        if not IsPedOnMount(playerped) and not IsPedInAnyVehicle(playerped) and not IsPedDeadOrDying(playerped) then
            ...
            local trap = DoesObjectOfTypeExistAtCoords(x, y, z, 50.0, GetHashKey(v), true)
            local trapclose = DoesObjectOfTypeExistAtCoords(x, y, z, 1.2, GetHashKey(v), true)
                if trap and not InArray(CheckedTraps, tostring(v)) then
                    
                    sleep = false
                    if active == false then
                        if trapclose then
                        local MiningGroupName  = CreateVarString(10, 'LITERAL_STRING', "Wood Trap")
                    ...
                    if PromptHasHoldModeCompleted(MinePrompt) then
                        ...
                        Citizen.Wait(500)
                        TriggerServerEvent("clout_crabs:hookcheck", tostring(v))
                    end
                else
                end
...
RegisterNetEvent("clout_crabs:hookchecked")
AddEventHandler("clout_crabs:hookchecked", function(trap)
    goTrap(trap)
end)

RegisterNetEvent("clout_crabs:nohook")
AddEventHandler("clout_crabs:nohook", function()
    active = false
end)

function goTrap(prop)
    EquipSpike(GetHashKey("p_hayhook01x"), 'Hook Trap')
    local swing = 0
    ...
    while hastool == true do
        ...
        if IsControlJustReleased(10, Config.StopMiningKey) or IsPedDeadOrDying(PlayerPedId()) then
            swing = 0
            table.insert(CheckedTraps, tostring(prop))
            hastool = false
        ...
        elseif IsControlJustPressed(0, Config.MineRockKey) then
            ...
            swing = swing + 1
            Anim(ped,'mech_dynamic@world_player_dynamic_kneel_ground@trans@kneel1@male_a','kneel1_trans_tend1',-1,0)
            local testplayer = exports["syn_minigame"]:taskBar(randomizer,7)
            if testplayer == 100 then 
                TriggerServerEvent('clout_crabs:addItem')
            end
        end
        ...
        if swing == swingcount then
            ...
            Citizen.CreateThread(function()
                Citizen.Wait(10000)
                table.remove(CheckedTraps, GetArrayKey(CheckedTraps, tostring(prop)))
...
function EquipSpike(toolhash, prompttext, holdtowork)
    hastool = false
    ...
    AttachEntityToEntity(tool, ped, GetPedBoneIndex(ped, 7966), 0.0,0.1,0.02,  -52.0,100.0,0.0, 0, 0, 0, 0, 2, 1, 0, 0);
    Citizen.InvokeNative(0x923583741DC87BCE, ped, 'guard_lantern')
